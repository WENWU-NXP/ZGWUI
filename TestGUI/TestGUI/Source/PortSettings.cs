...
namespace PortSet
{
    public partial class PortSettings : Form
    {
        ...
        {
            if (listBoxPorts.SelectedItems.Count == 1)
            {
            ...
            {
            //    Console.WriteLine("listBoxPorts.SelectedItems.Count: {0}", listBoxPorts.SelectedItems.Count);
                string[] strArray = DevInfo.DeviceInfo.ParsePorts();
                for (int i=0; i< listBoxPorts.SelectedItems.Count;i++)
                {                   
                    int index = listBoxPorts.SelectedIndices[i];

                    if (strArray[index] != "")
                    {
                        PortList.Add(strArray[index]);                    
                    }
                }
            }
